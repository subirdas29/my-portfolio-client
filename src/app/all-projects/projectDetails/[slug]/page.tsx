...
export async function generateMetadata({params}: {params: Promise<{slug: string}>}): Promise<Metadata> {
  ...
  return {
    ...
    description: project?.shortDescription || `Explore ${project?.title} - A ${project?.projectType} project by Subir Das`,
    keywords: project?.technologies?.join(", ") || "web development, project, portfolio",
    openGraph: {
      title: project?.title,
      description: project?.shortDescription || `Explore ${project?.title} - A ${project?.projectType} project by Subir Das`,
